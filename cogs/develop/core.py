...
class DevelopCog(commands.Cog, name="Develop"):
    ...
    async def extensions(self, interaction: Interaction) -> None:
        """Show All Extensions."""
        embed = ExtensionEmbed(interaction.user, self.bot, title="Extensions", color=Color.dark_grey())
    ...
    async def reload(self, interaction: Interaction, extension: str | None = None, sync: bool = True) -> None:
        ...
        embed = ExtensionEmbed(interaction.user, self.bot, title="Reloading...", color=Color.light_grey())
        await interaction.response.send_message(embed=embed, ephemeral=True)
        ...
        if not sync:
            ...
            embed = ExtensionEmbed(interaction.user, self.bot, title="Reloaded!", color=Color.green())
            await interaction.edit_original_response(embed=embed)
            return

        embed = ExtensionEmbed(interaction.user, self.bot, title="Syncing...", color=Color.light_grey())
        await interaction.edit_original_response(embed=embed)
        await self.bot.command_sync()

        embed = ExtensionEmbed(interaction.user, self.bot, title="Reloaded and Synced!", color=Color.green())
        await interaction.edit_original_response(embed=embed)
    ...
    async def sync(self, interaction: Interaction, force: bool = False) -> None:
        """Sync All Commands."""
        embed = CommandMapEmbed(interaction.user, self.bot, title="Syncing...", color=Color.light_grey())
        await interaction.response.send_message(embed=embed, ephemeral=True)
        await self.bot.command_sync(force=force)

        embed = CommandMapEmbed(interaction.user, self.bot, title="Synced!", color=Color.green())
        await interaction.edit_original_response(embed=embed)
    ...
    async def register(self, interaction: Interaction) -> None:
        ...
        embed = CommandMapEmbed(interaction.user, self.bot, title="Registering...", color=Color.light_grey())
        await interaction.response.send_message(embed=embed, ephemeral=True)
        await self.bot.command_map()

        embed = CommandMapEmbed(interaction.user, self.bot, title="Registered!", color=Color.green())
        await interaction.edit_original_response(embed=embed)
    ...
    async def map(self, interaction: Interaction) -> None:
        """Show Command Map."""
        embed = CommandMapEmbed(interaction.user, self.bot, title="Command Map", color=Color.dark_grey())
        await interaction.response.send_message(embed=embed, ephemeral=True)
    ...
    async def commands(self, interaction: Interaction) -> None:
        ...
        embed = utils.CustomEmbed(interaction.user, title="Command List", color=Color.dark_grey())
